...
<section className="relative h-screen flex flex-col justify-center items-center text-white overflow-hidden bg-black">
  ...
  {/* Capas de nebulosa tipo Hubble */}
  <div className="absolute inset-0 -z-10">
    {/* Capa principal de nebulosa morada-rosa */}
    <div className="absolute w-[220%] h-[220%] bg-gradient-to-r from-purple-600/60 via-pink-500/50 to-indigo-700/50 rounded-full blur-[350px] animate-nebulaMove"></div>
    {/* Capa secundaria azul-cyan */}
    <div className="absolute w-[200%] h-[200%] bg-gradient-to-r from-cyan-400/40 via-blue-500/40 to-purple-500/40 rounded-full blur-[300px] animate-nebulaMove2"></div>
    {/* Capa terciaria suave rosada */}
    <div className="absolute w-[240%] h-[240%] bg-gradient-to-r from-pink-400/30 via-purple-400/30 to-blue-600/30 rounded-full blur-[280px] animate-nebulaMove3"></div>
  </div>

  {/* Estrellas parpadeantes */}
  {[...Array(100)].map((_, i) => {
    const size = Math.random() * 2 + 1; // tamaño variable
    const left = Math.random() * 100;
    const top = Math.random() * 100;
    const delay = Math.random() * 5;
    const duration = Math.random() * 3 + 2;
    const color = ["#fff","#ffe","#aff","#f0f"][Math.floor(Math.random()*4)];
    return (
      <motion.span
        key={i}
        className="absolute rounded-full"
        style={{ width: size, height: size, left: `${left}%`, top: `${top}%`, backgroundColor: color, opacity: 0.8 }}
  ...
  {/* Nombre con efecto de encendido tipo cartel */}
  <h1
    className="relative text-5xl md:text-7xl lg:text-8xl font-extrabold uppercase tracking-wide text-center select-none z-10"
  ...
  {/* CSS interno para nebulosa y efecto de letras */}
  <style jsx>{`
    .text-neon {
      color: #0ff;
      text-shadow: 0 0 2px #0ff, 0 0 5px #0ff, 0 0 10px #0ff;
      opacity: 0;
      animation: neonOn 1.5s forwards;
    }

    @keyframes neonOn {
      0% { opacity: 0; text-shadow: none; }
      50% { opacity: 1; text-shadow: 0 0 2px #0ff, 0 0 5px #0ff, 0 0 10px #0ff; }
      100% { opacity: 1; text-shadow: 0 0 2px #0ff, 0 0 5px #0ff, 0 0 10px #0ff; }
    }
    ...
    .animate-nebulaMove {
      animation: nebulaMove 50s ease-in-out infinite alternate;
    }

    .animate-nebulaMove2 {
      animation: nebulaMove2 60s ease-in-out infinite alternate;
    }

    .animate-nebulaMove3 {
      animation: nebulaMove3 70s ease-in-out infinite alternate;
    }
  `}</style>
</section>
