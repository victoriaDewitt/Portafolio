...
></div>
 ...
 <section className="relative h-screen flex flex-col justify-center items-center bg-gradient-to-br from-blue-800 via-purple-900 to-black text-white overflow-hidden">
  ...
  {[...Array(50)].map((_, i) => {
    const size = Math.random() * 4 + 2; // Tamaño más visible
    const left = Math.random() * 100; // Posición horizontal
    const delay = Math.random() * 10; // Diferente velocidad inicial
    const duration = Math.random() * 5 + 5; // Velocidad caída
    return (
      <motion.div
        key={i}
