...
{/* Banda de Skills */}
<div className="relative w-full py-20 z-10 bg-black/10 overflow-hidden">
  <div className="flex animate-marquee space-x-20 justify-center">
    {skillIcons.concat(skillIcons).map((skill, index) => (
      <div key={index} className="relative flex flex-col items-center group min-w-[160px]">
        {/* Tooltip */}
        <span
          className="absolute -top-12 px-3 py-1 rounded text-xl font-bold text-cyan-400 opacity-0 group-hover:opacity-100 select-none"
          style={{
        ...
        <img
          src={skill.icon}
          alt={skill.name}
          className="h-48 w-48 md:h-56 md:w-56 object-contain transition-transform duration-300 group-hover:-translate-y-8"
        />
      </div>
    ))}
  </div>
