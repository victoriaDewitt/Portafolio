...
function App() {
  ...
  const navLinks = [
      ...
      { href: "#education", text: t.education },
  ...
  return (
    <div className="font-sans bg-black text-gray-100 relative">
      ...
      {/* Navbar */}
      <header className="bg-gradient-to-r from-indigo-800 via-purple-900 to-pink-800/90 z-30 sticky top-0">
        <nav className="max-w-6xl mx-auto flex justify-between items-center p-4">
          ...
          <ul className="hidden md:flex gap-6 text-lg font-medium">
            {navLinks.map((link) => (
                <li key={link.href}>
                    <a href={link.href} className="hover:text-indigo-300">
                        {link.text}
                    </a>
                </li>
            ))}
      ...
      <motion.nav 
          ...
          variants={{
              ...
              closed: { opacity: 0, y: -50, height: 0, transition: { duration: 0.3 } },
          }}
          className={`absolute top-[64px] left-0 w-full bg-indigo-900/95 backdrop-blur-sm shadow-xl z-20 ${isMenuOpen ? 'block' : 'hidden md:hidden'}`}
      >
          <ul className="flex flex-col items-center gap-4 py-4 text-xl font-medium">
              {navLinks.map((link) => (
                  <li key={link.href} className="w-full text-center">
                      <a 
                          href={link.href} 
                          className="block py-2 hover:bg-indigo-700/50 w-full"
      ...
      {/* Hero Section */}
      {/* ... (Todo el contenido siguiente permanece igual, usando 't' para las traducciones) ... */}
