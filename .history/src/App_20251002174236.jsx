          ...
          <div className="flex animate-marquee space-x-8 justify-center">
            {skillIcons.concat(skillIcons).map((skill, index) => (
              <div key={index} className="relative flex flex-col items-center group min-w-[100px]">
                <span className="absolute -top-10 px-2 py-1 rounded text-lg font-bold text-cyan-400 opacity-0 group-hover:opacity-100 select-none" style={{ textShadow: '0 0 2px #0ff, 0 0 2px #0ff, 0 0 3px #0ff', transition: 'all 0.25s ease-in-out' }}>{skill.name}</span>
