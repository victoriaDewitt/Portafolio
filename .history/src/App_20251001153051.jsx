...
<section id="about" className="relative min-h-screen flex items-center justify-center px-6 py-16 overflow-hidden text-white">
  ...
  <div className="absolute inset-0 -z-20">
    ...
    <div className="absolute w-[240%] h-[240%] rounded-full blur-[220px] animate-nebulaMove3" style={{
  ...
  {[...Array(100)].map((_, i) => {
    ...
    return (
      <motion.span
        ...
        style={{ width: size, height: size, left: `${left}%`, top: `${top}%`, backgroundColor: color }}
        animate={{ opacity: [0.2, 1, 0.2], x: [0, Math.random() * 10 - 5, 0], y: [0, Math.random() * 10 - 5, 0] }}
        transition={{ duration, delay, repeat: Infinity, ease: "easeInOut" }}
      />
    );
  })}

  {/* Esferas flotantes grandes y pequeñas */}
  {[...Array(40)].map((_, i) => {
    const size = Math.random() * 30 + 10;
    const left = Math.random() * 100;
    const top = Math.random() * 100;
    const opacity = size > 20 ? 0.1 : 0.3;
