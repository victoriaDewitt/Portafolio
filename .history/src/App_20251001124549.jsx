...
<section className="relative h-screen flex flex-col justify-center items-center text-white overflow-hidden">
  ...
  {/* Fondo de nebulosa con gradiente animado */}
  <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_20%_30%,_#ff70a6,_transparent_40%),radial-gradient(circle_at_80%_70%,_#845ec2,_transparent_50%),radial-gradient(circle_at_50%_50%,_#0081cf,_transparent_60%),#0b0b1d] bg-[length:200%_200%] animate-nebulaMove" />
  ...
  {[...Array(50)].map((_, i) => {
    ...
    return (
      <motion.span
        ...
        style={{ width: size, height: size, left: `${left}%`, top: `-5%` }}
        animate={{ top: ["-5%", "105%"] }}
  ...
  {/* Nombre con efecto neon tipo cartel */}
  <h1 className="relative text-5xl md:text-7xl lg:text-8xl font-extrabold uppercase tracking-wide text-center select-none z-10" style={{ fontFamily: 'Orbitron, sans-serif' }}>
    Hi, I'm{" "}
    <span>
      {name.split("").map((letter, i) => (
        <span key={i} className={`text-neon startup`} style={{ animationDelay: `${i * 0.1}s` }}>
          {letter}
        </span>
      ))}
    </span>
  ...
  {/* CSS para efecto neon tipo cartel + movimiento nebulosa */}
  <style jsx>{`
    .text-neon {
      color: #0ff;
      text-shadow:
    ...
    @keyframes startupFlicker {
      0%, 8% { opacity: 0; text-shadow: none; }
      10%, 18% { opacity: 1; text-shadow: 0 0 3px #0ff, 0 0 6px #0ff; }
      20%, 25% { opacity: 0; text-shadow: none; }
      30%, 50% { opacity: 1; text-shadow: 0 0 4px #0ff, 0 0 8px #0ff; }
      60% { opacity: 0.6; text-shadow: 0 0 2px #0ff; }
      70%, 100% {
        opacity: 1;
        text-shadow:
          0 0 3px #0ff,
    ...
    .startup {
      animation: startupFlicker 3s forwards;
    }
