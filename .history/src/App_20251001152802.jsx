...
<section id="about" className="relative min-h-screen flex items-center justify-center px-6 py-16 overflow-hidden text-white">
  
  {/* Fondo tipo nebulosa */}
  <div className="absolute inset-0 -z-20">
    <div className="absolute w-[220%] h-[220%] rounded-full blur-[250px] animate-nebulaMove" style={{
      background: 'radial-gradient(circle at 30% 40%, #0d1b2a, transparent 70%)',
      opacity: 0.4,
      mixBlendMode: 'screen'
    }} />
    <div className="absolute w-[200%] h-[200%] rounded-full blur-[200px] animate-nebulaMove2" style={{
      background: 'radial-gradient(circle at 70% 50%, #1b263b, transparent 70%)',
      opacity: 0.35,
      mixBlendMode: 'screen'
    }} />
  </div>
  ...
  {[...Array(50)].map((_, i) => {
    ...
    const left = Math.random() * 100;
    const top = Math.random() * 100;
    const color = ["#ffffff","#c0c0ff","#a0ffff"][Math.floor(Math.random()*3)];
    return (
      <motion.span
        key={i}
        className="absolute rounded-full"
  ...
  {/* Partículas que caen */}
  {[...Array(60)].map((_, i) => {
    const size = Math.random() * 15 + 5; // esferas grandes y pequeñas
    const left = Math.random() * 100;
    const delay = Math.random() * 10;
    const duration = Math.random() * 15 + 10;
    const opacity = size > 10 ? 0.15 : 0.5; // grandes más difuminadas, pequeñas más visibles
    return (
      <motion.div
        key={i}
        className="absolute rounded-full bg-white"
        style={{ width: size, height: size, left: `${left}%`, top: `-20%`, opacity }}
        animate={{ top: ["-20%", "120%"] }}
  ...
  <div className="relative max-w-6xl w-full grid md:grid-cols-2 gap-12 items-center z-10">
    ...
    >
      <img 
        src="/images/vicky.jpg"  // 🔄 tu ruta real
