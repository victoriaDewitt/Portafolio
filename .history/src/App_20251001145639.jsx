...
<section className="relative h-screen flex flex-col justify-center items-center overflow-hidden">
  ...
  <div className="absolute inset-0">
    <div className="absolute w-[220%] h-[220%] rounded-full blur-[350px] animate-nebulaMove" style={{
      background: 'radial-gradient(circle at 30% 40%, #1e3a8a, transparent 70%)', // azul oscuro
      opacity: 0.6,
      mixBlendMode: 'screen'
    }} />
    <div className="absolute w-[200%] h-[200%] rounded-full blur-[300px] animate-nebulaMove2" style={{
      background: 'radial-gradient(circle at 70% 50%, #db2777, transparent 60%)', // rosa profundo
      opacity: 0.5,
      mixBlendMode: 'screen'
    }} />
    <div className="absolute w-[240%] h-[240%] rounded-full blur-[280px] animate-nebulaMove3" style={{
      background: 'radial-gradient(circle at 50% 70%, #7c3aed, transparent 70%)', // violeta
      opacity: 0.4,
      mixBlendMode: 'screen'
    }} />
  </div>
  ...
  {/* Estrellas */}
{[...Array(300)].map((_, i) => { // aumentamos a 300 estrellas
  const size = Math.random() * 2 + 0.3; // tamaños variados
  const left = Math.random() * 100;
  const top = Math.random() * 100;
  const delay = Math.random() * 5;
  const duration = Math.random() * 3 + 2;
  const color = ["#ffffff","#c0c0ff","#ffa0ff","#a0ffff","#ffd700"][Math.floor(Math.random()*5)]; // agregamos dorado tenue
  return (
    <motion.span
      key={i}
